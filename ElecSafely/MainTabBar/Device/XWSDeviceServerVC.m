...
- (void)loadData {
    NSDictionary *para = @{@"ID":self.baseInfoData.ID?:@"",@"SD":@"",@"ED":@""};
...
- (void)createUI {
    _serverTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight) style:UITableViewStylePlain];
    _serverTableView.dataSource = self;
    _serverTableView.delegate = self;
    _serverTableView.backgroundColor = CURRENT_VC_BACKCOLOR;
    _serverTableView.rowHeight = 60.f;
